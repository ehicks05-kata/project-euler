...
public class Problem11
{
    ...
    public static void findGreatestProduct()
    {
        ...
        for(int row = 0; row < grid.length; row++)
        {
            for(int column = 0; column < grid[row].length; column++)
            {
                ...
                if(column < grid[row].length - 3 && row > 3) //UPRIGHT
                {
                    product = grid[row][column] * grid[row-1][column+1] * grid[row-2][column+2] * grid[row-3][column+3];
                    highestProduct = isHighest(product, highestProduct);
                }
    ...
    public static int isHighest(int pro, int highestPro)
    {
        if(pro > highestPro)
        {
            return pro;
    ...
    public static int findProduct(int x, int y, String direction)
    {
        int product = 0;
        return product;
    }
